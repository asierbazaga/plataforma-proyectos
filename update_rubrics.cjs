...
// I will keep Framework and Softskills.
// I will strip the existing ones by parsing it or just creating a completely new file and copying EVALUATION_LEVELS and INITIAL_CANDIDATE_SAMPLE.

const newRubrics = `import { MecaluxCompetencyRubric } from '../../../types';
...
// Now keep the Softskills and Framework from original.
...
// Remove ss_trabajo_equipo from the original if it exists
let restOfCode = restMatch[0];
// Ensure we don't duplicate trabajo en equipo
restOfCode = restOfCode.replace(/\{\s*id:\s*'ss_trabajo_equipo'[\s\S]*?\},?\s*/g, '');

const finalFileContent = newRubrics + restOfCode;
