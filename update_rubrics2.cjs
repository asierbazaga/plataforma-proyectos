...
const regex = /export const MECALUX_RUBRICS: MecaluxCompetencyRubric\[\] = \[([\s\S]*?)\];\s*export const EVALUATION_LEVELS/m;
...
const rubricsText = match[1];

const newRubrics = `import { MecaluxCompetencyRubric, CandidateInterview, MecaluxEvaluationLevel } from '../../../types';
...
const restMatch = code.match(/(\{\s*id:\s*'(fw_|ss_)[^]*)/);
if (!restMatch) {
  console.error("Could not find the rest of rubrics!");
  process.exit(1);
}

let restOfCode = restMatch[0];

// Correctly remove ss_trabajo_equipo block which spans multiple objects or arrays:
// Instead of regex, I'll just remove it logically.
// The easiest regex for a known block is from '{ id: 'ss_trabajo_equipo'' to the end of its 'disparadores' array ']' and its closing '}'
restOfCode = restOfCode.replace(/\{\s*id:\s*'ss_trabajo_equipo'[\s\S]*?disparadores:\s*\[[\s\S]*?\]\s*\},?\s*/, '');


// Because I am writing to the file directly from scratch, I also need to make sure INITIAL_CANDIDATE_SAMPLE uses import.
// It is at the end of the file.
// The original file imports were removed if I just prepend newRubrics.
// Let's use string replacement to just replace the CP section and let the rest untouched.

const startMarker = '// COMPETENCIAS PROFESIONALES (TÉCNICAS & OPERATIVAS MECALUX)';
const endMarker = '  // ==========================================================================';

// Replace the CP block in the original code
const cpStart = code.indexOf('{', code.indexOf('cp_intralogistica'));
const fwStart = code.indexOf('{', code.indexOf('fw_'));

let finalFile = code.substring(0, cpStart); // up to first CP object
// insert new rubrics (without the "export const" stuff, just the objects)
let newObjectsOnly = newRubrics.substring(newRubrics.indexOf('{', newRubrics.indexOf('cp_trabajo_equipo')));

finalFile += newObjectsOnly;
let fwCode = code.substring(fwStart);

// remove ss_trabajo_equipo from fwCode
fwCode = fwCode.replace(/\{\s*id:\s*'ss_trabajo_equipo'[\s\S]*?disparadores:\s*\[[\s\S]*?\]\s*\},?\s*/, '');

finalFile += fwCode;

fs.writeFileSync(filepath, finalFile);
console.log('Successfully updated the rubrics file');
