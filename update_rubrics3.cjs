...
const startIndex = code.indexOf('  // COMPETENCIAS PROFESIONALES');
...
// Also remove ss_trabajo_equipo from the rest of the code since it was moved to cp_trabajo_equipo
const cleanedCode = newCode.replace(/\{\s*id:\s*'ss_trabajo_equipo'[\s\S]*?disparadores:\s*\[[\s\S]*?\]\s*\},?\s*/, '');
